void Ave137(const char* run="Integrated_20220722182222_1.root")
{
	TChain *t = new TChain("t");
	t->Add(Form("./Integrated_20220722182222_1.root"));
	...
	for (int i=0; i<nevt; i++) { // event loop
		...
		if(db<0.55 && b>1100 && is<1 && h/a<0.01 && a>148e4 && a<156e4) { //cuts in the 662 keV region
	...
	// overshoot model
	TF1 *f = new TF1("f",
			"[0]*([1]*exp(-(x-[3])/[2])-[2]*exp(-(x-[3])/[1]))/([1]-[2])",200,3000);
	f->SetParNames("Norm","#tau_{i}", "#tau", "Start");
	f->SetLineColor(kRed);
	f->SetParameter(0,8170);
	f->SetParLimits(0, 7500,8500);
	...
	latex.DrawLatex(500,5000, Form("662 keV peak area after correction: %.0f",A));

		can->Print("AveCs137.png");
	gSystem->Chmod("AveCs137.png",S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP);
}
