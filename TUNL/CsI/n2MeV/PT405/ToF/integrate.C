...
void integrate(const char* run="SIS3316Raw_20240802231628_1.root")
{
	int n, offset=0; bool is, pu; unsigned long long ts0deg, tsBPM;
	float a, ah, b, db, f, h, tt, th, dt;
	float s[526], t[526], sb[526], tb[526];

	TFile *input = new TFile(run);

	TTree *t0deg = (TTree*) input->Get("t0");
	...
	TTree *tBPM = (TTree*) input->Get("t15");
	...
	to->Branch("sb",sb,"sb[n]/F"); // BPM sample in unit of ADC
	to->Branch("b",&b,"b/F"); // baseline of BD averaged over 160 samples
	...
	for (int i=0; i<nevt; i++) {
		...
		for (int k=0; k<160; k++) b+=s[k]; b/=160; // calculate baseline
		for (int k=0; k<n; k++) {
			if (s[k]>16382) is=kTRUE; // saturated (reached 2^14-1)
			if (k<160) db+=(s[k]-b)*(s[k]-b); // calculate baseline RMS
			s[k]-=b; // remove baseline
			if (s[k]>20 && tt<0) tt=k; // software trigger time
		}
		db=sqrt(db)/160; // RMS of baseline
